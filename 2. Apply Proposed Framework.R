...
AllMet <- data.frame(t(rep(0,30)))
colnames(AllMet) <- c("Dataset","Size","Rare","RareP","NumVar","CatVar","TotVar","FinVar",
                      "KNN.US","SVM.US","AEN.US","KNN.SS","SVM.SS","AEN.SS","HBOS","RAF",
                      "U101.US","U110.US","U111.US","S101.SS","S110.SS","S111.SS","Fin.US","Fin.SS",
                      "Metric","Var1","Var2","Var3","Var4","Var5")  
AllTmp <- data.frame(t(rep(0,30)))
...
run_master <- function(XNumer, XCateg, inputDat, DataType){
  ...
  for (iter in 1:Iterations){
    ...
    rm(variance)
    trainX <- trainP[,1:max(3,length(which(cumvar<99)))]
    testsX <- testsP[,1:max(3,length(which(cumvar<99)))]
    rm(cumvar)
    rm(trainP)
    rm(testsP)
    ...
    rm(trainN)
    rm(testsN)
    IntTest$U101.US <- IntTest$KNN.US*1 + IntTest$SVM.US*0 + IntTest$AEN.US*1
    IntTest$U110.US <- IntTest$KNN.US*1 + IntTest$SVM.US*1 + IntTest$AEN.US*0
    IntTest$U111.US <- IntTest$KNN.US*1 + IntTest$SVM.US*1 + IntTest$AEN.US*1
    IntTest$S101.SS <- IntTest$KNN.SS*1 + IntTest$SVM.SS*0 + IntTest$AEN.SS*1
    IntTest$S110.SS <- IntTest$KNN.SS*1 + IntTest$SVM.SS*1 + IntTest$AEN.SS*0
    IntTest$S111.SS <- IntTest$KNN.SS*1 + IntTest$SVM.SS*1 + IntTest$AEN.SS*1
    if (AllTmp[1,4] >= 0.01) {
      if (DataType == "M") {
        IntTest$Fin.US <- IntTest$U101.US
        IntTest$Fin.SS <- IntTest$S101.SS
      } else {
        IntTest$Fin.US <- IntTest$KNN.US
        IntTest$Fin.SS <- IntTest$KNN.SS
      }
    } else {
      if (DataType == "M") {
        IntTest$Fin.US <- IntTest$U111.US
        IntTest$Fin.SS <- IntTest$S111.SS
      } else {
        IntTest$Fin.US <- IntTest$U110.US
        IntTest$Fin.SS <- IntTest$S110.SS
      }
    }
    
    F1Ar <- rep(0,16)
    # Report the best F1 Score for each method
    for (rocCut in 1:25) {
      for (perf in 1:16) {
        IntTest[,18] <- base::factor(ifelse(IntTest[,(perf+1)] < quantile(IntTest[,(perf+1)],(1-min(0.8,(AllTmp[1,4]/2*rocCut))), na.rm=TRUE),0,1))
        predAll <- IntTest[IntTest$V18 == 1,, drop=FALSE]
        levels(predAll$V18) <- c("0","1")
        levels(predAll$Y) <- c("0","1")
        actAll <- IntTest[IntTest$Y == 1,, drop=FALSE]
        levels(actAll$V18) <- c("0","1")
        levels(actAll$Y) <- c("0","1")
        Prec <- sum(as.numeric(predAll$V18 == predAll$Y))/nrow(predAll)
        Recc <- sum(as.numeric(actAll$V18 == actAll$Y))/nrow(actAll)
        rm(predAll)
        rm(actAll)
        F1it <- ifelse(is.nan(2*Prec*Recc / (Prec + Recc)) | is.na(2*Prec*Recc / (Prec + Recc)),0,2*Prec*Recc / (Prec + Recc))
        if (F1it > F1Ar[perf]) { 
          F1Ar[perf] <- round(F1it,4)
          IntTest[,19] <- IntTest[,18]
        }
      }
    }
    All2 <- rbind(AllTmp,AllTmp)
    All2[1,9:25] <- cbind(t(F1Ar),"F1")
    rm(F1Ar)
    rm(F1it)
    rm(rocCut)
    rm(perf)
    AUCi <- c(round(roc(IntTest$Y,IntTest$KNN.US)$auc*1,4),round(roc(IntTest$Y,IntTest$SVM.US)$auc*1,4),round(roc(IntTest$Y,IntTest$AEN.US)$auc*1,4),
              ...
              round(roc(IntTest$Y,IntTest$U101.US)$auc*1,4),round(roc(IntTest$Y,IntTest$U110.US)$auc*1,4),round(roc(IntTest$Y,IntTest$U111.US)$auc*1,4),
              round(roc(IntTest$Y,IntTest$S101.SS)$auc*1,4),round(roc(IntTest$Y,IntTest$S110.SS)$auc*1,4),round(roc(IntTest$Y,IntTest$S111.SS)$auc*1,4),
              round(roc(IntTest$Y,IntTest$Fin.US)$auc*1,4),round(roc(IntTest$Y,IntTest$Fin.SS)$auc*1,4))
    All2[2,9:25] <- cbind(t(AUCi),"AUC")
    dd <- data.frame(testsD,IntTest[,19,drop=FALSE])
    rm(testsD)
    binning <- data.frame(woe.binning(dd,"V19",dd))
    ab <- binning[,c(1,3)]
    vars <- unlist(ab$X1)
    var2 <- unlist(ab$X3)
    rm(ab)
    All2[1,26:30] <- vars[1:5]
    All2[2,26:30] <- round(var2[1:5],4)
    rm(vars)
    rm(var2)
    if (iter == 1) {
...
TrainPerc <- 60 # Defines random split for train and test data
Iterations <- 30 # number of repititions to report average performance
# Data Pre-requisites:
...
# 5. inputDat specifies the file name (without .csv extension)
# 6. DataType specifies the majority predictor type (M: Machine-related, H: Human-related)
perF <- run_master(XNumer = 5, XCateg = 6, inputDat = "myData", DataType = "M")
write.csv(perF, "perfSumm.csv", row.names = FALSE)
